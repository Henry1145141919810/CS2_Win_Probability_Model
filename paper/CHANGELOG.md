...
- Each version is also a **git tag** (`paper-vN`), so `git diff paper-v1 paper-v2 -- docs/paper/`
  (the paper lived in `docs/paper/` up to v13 and in `paper/` from v13.1; across the move use `git diff -M`)
...
`docs/cluster/BETTY_benchmark_guide.md`); GAT out-of-time (needs a 2026 trajectory dataset). Task A (2026
...
Full method + results + documented snapshot selection: `docs/studies/study_beyond_economy_results.md`. Code:
...
**Notes/data.** `docs/studies/notes_pathwise_calibration.md`, `outputs/pathwise_benchmark.csv`,
...
nothing). Conclusion unchanged, better supported. See `paper/TODO_firepower_rewrite.md`.
...
Tag: `paper-v13` · Zip: `CS2_winprob_overleaf_v13.zip`
...
- **Sect. "How much of the model is economy? The no-economy ablation"** (`sec:noecon`, after the model
  ...
  results `docs/studies/study_noecon_results.md`.
- **Sect. "The round-start prior"** (`sec:prior`, after the trajectory-honesty paragraph): round-level
  ...
  `src/models/t0_table.py`, `src/viz/t0_examples.py`; plan `docs/studies/plan_study1_t0_prior.md`.
...
study (`docs/studies/study_round_to_map_results.md`) is not in the paper yet (a separate section once the
known-truth simulation is done).

## v13.1 — 2026-09-23 — compile fix; paper moved to `paper/`; first public preprint PDF

Tag: `paper-v13.1` · Zip: `CS2_winprob_overleaf_v13.1.zip` (local, gitignored) · PDF: `paper/CS2_Win_Probability_preprint.pdf`

**Changed.** v13 did not compile: the round-start subsection used `\mathbb{E}`, which needs `amssymb`,
not loaded by the preamble. Replaced with `\mathrm{E}` (three occurrences); no other text changed. The
paper directory moved from `docs/paper/` to `paper/` as part of the repository reorganization, and the
compiled preprint (43 pages, built with Tectonic 0.17, zero unresolved references or citations) is now
committed.

**Re-upload to Overleaf.** main.tex only (if v13 was uploaded, it failed to compile there for the reason
above).
