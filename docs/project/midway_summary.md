...
## 6. Evaluation framework (full battery — see `docs/methodology/metrics.md`)
...
1. **🔴 THE ONLY BLOCKING TASK — 2026 HLTV scrape (Leu).** Full spec:
   **`docs/project/TODO_leu_2026_scrape.md`**. Two scrapes: **2026 stats for all 82** players (same-year
...
4. **Paper draft** — methods + results are complete; `docs/studies/results_checkpoint.md` + `docs/methodology/metrics.md`
...
| Deep models (Betty) | `src/models/deep/{tcn,gat,transformer}.py`, jobs `jobs/*.sh`, runbook `docs/cluster/cluster_runbook.md` |
| Docs | `docs/{results_checkpoint,metrics,methodology,firepower_pillar,midway_summary}.md` |

**Single best reference for current numbers:** `docs/studies/results_checkpoint.md` (the model matrix) and
`docs/methodology/metrics.md` (every metric + values + interpretation).
