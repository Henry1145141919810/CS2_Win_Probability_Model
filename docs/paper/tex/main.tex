...
statistically indistinguishable --- a dead heat that persists out-of-time, where the sequence
models land exactly on the classical number.
...
Repairing the dependency across three constructions --- a leak-free \emph{lagged}
prior, and even a leaky same-era \emph{oracle} at 100\,\% coverage --- restores
calibration completely, yet the skill pillar \emph{still} fails to beat the
demo-only model out-of-time under \emph{any} of them. This turns a plumbing bug
into a rigorous negative result: even a correctly-constructed, coverage-complete
external skill prior is not worth its cost here.
\end{abstract}
