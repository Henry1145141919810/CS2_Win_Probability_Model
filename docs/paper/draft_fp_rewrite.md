> **Variant of `draft.md`.** The firepower pillar is restructured from a version history
> (v1 → v2 → v3) into four parallel encodings evaluated on a common skill-free base.
> The conclusion is unchanged; it is now reached by systematic ablation rather than by
> assertion. Diverges only in §4.4, §7.5 and one Limitations item.
> Checklist: [`TODO_firepower_rewrite.md`](TODO_firepower_rewrite.md).
