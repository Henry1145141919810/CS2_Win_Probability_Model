...
```bash
kinit hyhuang@UPENN.EDU              # realm MUST be uppercase; ticket lasts 10 h
ssh hyhuang@login.betty.parcc.upenn.edu   # -> Duo push on first factor pairing
```

Login nodes: `login01/02/03.betty.parcc.upenn.edu` behind the `login.betty` alias (165.123.216.41-43).
...
`ssh-copy-id hyhuang@login.betty.parcc.upenn.edu`; then key + Kerberos ticket satisfies 2-of-3.
...
| Path | Purpose | Notes |
|---|---|---|
| `/vast/home/c/hyhuang` (`~`) | configs, code, conda envs (`~/envs/cs2-rwp`) | 50 GB; code only |
...
`parcc_sreport.py --user hyhuang`, failures with `parcc_sdebug.py --job <id>`.
...
- Small files (< ~1 GB): `scp file hyhuang@login.betty.parcc.upenn.edu:/vast/projects/ajw/wharton/cs2-rwp/data/`
...
`kerberos1-4.upenn.edu`, confirmed by DNS SRV records); creates Linux user `hyhuang` (= PennKey) with
